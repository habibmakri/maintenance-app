...
<body dir="rtl">
    ...
    <div style="margin-bottom: 7%;font-weight:bold;"class="header">
        <p style="margin: 0px; font-size:18px;">الدورة رقم {{ $list->counter }}
            {{ $types[$list->type] }}</p>
        <p style="margin: 0px;font-size:18px;">ورقة النتائج: الفوج{{ $group }}</p>
    </div>
    ...
    <table style="font-size:14px;">
        <thead>
            <tr>
                ...
                <td style="font-weight: bold;width:25%">نقطة المواظبة</td>
                <td style="font-weight: bold;width:20%">نقطة الإمتحان</td>
            </tr>
        </thead>
        <tbody>
            @php
