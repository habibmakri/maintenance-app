...
<body dir="rtl" >
    <div style="margin-bottom: 7%;"class="header">
        <p style="margin: 0px; font-size:18px;">القائمة رقم {{$list->counter}} المترشحين المقبولين لنيل شهادة التكوين الخاصة بسائقي سيارة الأجرة</p>
    </div>
    ...
    <table style="font-size:14px;">
        ...
        <tbody>
            @php
                $i = 1;
            @endphp
            @foreach ($list->count_taxis as $taxi)
                <tr>
                    <td>{{$i}}</td>
                    <td>{{$taxi->nom_ar}}</td>
