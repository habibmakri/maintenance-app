@extends('base')
@section('title', 'Cartes gasoiles')
@section('content')

    <div class="pagetitle">
        <h1>Cartes gasoiles</h1>
        <nav>
            <ol class="breadcrumb">
        ...
        <div class="text-end">
            <a href="{{ route('app.gestion.add_carte_gasoile') }}" class="btn btn-primary">
                Nouvelle Carte gasoile
            </a>
        ...
        <tbody>
            ...
            @foreach ($cartes as $carte)
                <tr>
                    ...
                    <td>{{ $carte->number }}</td>
                    <td>{{ $carte->name }}</td>
                    <td>{{ $carte->initial_balance }}</td>
                    <td>{{ $carte->actual_balance }}</td>
                    <td>{{ $carte->state }}</td>
                    {{-- <td>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#ExtralargeModal"
                            onclick="handlerechargeclick({{ $carte }})">Recharger</button>
                    </td> --}}
                </tr>
                @php
                    $i = $i + 1;
