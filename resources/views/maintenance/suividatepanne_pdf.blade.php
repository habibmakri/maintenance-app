...
<body>
    <div style="margin-bottom: 7%;margin-left: 20%;"class="header">
        ...
        <h3>FICHE DES DATE DÉCLARATION ET RÉPARATION DES PANNES DÉCLARÉ</h3>
        <h2>{{ $monthName }}</h2>
    </div>
    @php
        ...
        <table style="font-size:10px;">
            <thead>
                <tr>
                    ...
                    <td style="font-weight: bold;width:25%">Panne/Travaux Effectués</td>
                    <td style="font-weight: bold;width:10%">Nature de Panne</td>
                    <td style="font-weight: bold;width:15%">Equipe</td>
                    <td style="font-weight: bold;width:15%">Moment/Lieu de l'opération</td>
                </tr>
            </thead>
            <tbody>
                @foreach ($pannes as $panne)
                    <tr>
                        <td>{{ \Carbon\Carbon::parse($panne['datedeclaration'])->format('d/m/Y') }}</td>
                        ...
                        <td style="text-align: left; text-align:justify;padding:5px;"><span
                                style="text-decoration: underline;"><strong>{{  $panne['name'] }}</strong></span><br>
                        </td>
                        <td>
                            {{ ucfirst($panne['type']) }}
                        </td>
